...
from hwt.simulator.utils import allHConstsToInts
...
class AxiReadAggregator_1word_burst_TC(SimTestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.dut = dut = AxiReadAggregator()
        dut.ID_WIDTH = 2
        dut.CACHE_LINE_SIZE = 4
        dut.DATA_WIDTH = 4 * 8
        cls.ID_CNT = 2 ** dut.ID_WIDTH
        cls.WORD_SIZE = dut.DATA_WIDTH // 8
        cls.compileSim(dut)
    ...
    def test_nop(self, randomized=False):
        ...
        self.assertEmpty(dut.s.r._ag.data)
    ...
    def _test_read(self, id_addr_tuples, randomize=False, REF_DATA=0x1000, time_multiplier=2):
        ...
        trans_len = ceil(dut.CACHE_LINE_SIZE / WORD_SIZE)
        ...
        ref_data = sorted(flatten([
            ...
            level=1))
        data = sorted(allHConstsToInts(dut.s.r._ag.data))
        # print("")
        # print(ref_data)
        # print(data)
    ...
    def _test_read_from_random_addr(self, address_cnt, N, randomize=False):
        available_addresses = [i * self.dut.CACHE_LINE_SIZE for i in range(address_cnt)]
        rand = self._rand
    ...
    def test_read_from_sequential_addr_2x(self, randomize=False):
        d_addr_tuples = [(x % self.ID_CNT, x * self.dut.CACHE_LINE_SIZE) for x in range(2)]
        self._test_read(d_addr_tuples, randomize)
