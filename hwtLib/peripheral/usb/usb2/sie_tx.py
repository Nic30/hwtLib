...
class Usb2SieDeviceTx(Unit):
    ...
    def _impl(self):
        ...
        tx_cmd: Usb2SieDeviceTxInput = AxiSBuilder(self, self.tx_cmd).buff(1).end
        ...
        If(~self.enable | (tx_cmd.ready & tx_cmd.valid & tx_cmd.last),
           is_sof_of_command(1)
        ).Elif(tx_cmd.ready & tx_cmd.valid,
           is_sof_of_command(0),
        )
        ...
        StreamNode(
            ...
            extraConds={
                deparser_hs.dataIn.pid:pid_is_hs & is_sof_of_command,
                deparser_data.dataIn.pid: pid_has_data & is_sof_of_command,
            },
            skipWhen={
                deparser_hs.dataIn.pid: pid_has_data,
                deparser_data.dataIn.pid: pid_is_hs,
            }
        ).sync(tx_cmd.valid & self.enable)

        crc16_valid = self._reg("crc16_valid", def_val=0)
        payload_in: AxiStream = deparser_data.dataIn.data
        StreamNode(
            [], [payload_in],
        ).sync(tx_cmd.valid & ~crc16_valid)

        # tx_cmd consume
        StreamNode(
            [tx_cmd], [],
            extraConds={
                tx_cmd: (pid_is_hs & deparser_hs.dataIn.pid.rd) |
                        (pid_has_data_zero_len & payload_in.ready & ~crc16_valid) |
                        (pid_has_data_non_zero_len & payload_in.ready & ~crc16_valid)
            }
        ).sync()
        ...
        If(end_of_payload | pid_has_data_zero_len,
           crc16_valid(~deparser_data.dataIn.crc16.rd),
        ...
        deparser_data.dataIn.crc16.vld(crc16_valid)
