...
class AvalonMM(Interface):
    ...
    def _declr(self):
        ...
        self.address = Param(self.ADDR_WIDTH)
        
        self.byteEnable = VectSignal(self.DATA_WIDTH // 8)
        self.read = Signal()
        self.readData = VectSignal(self.DATA_WIDTH)
        self.readValid = Signal(masterDir=DIRECTION.IN)  # read data valid
        self.response = VectSignal(2)

        self.write = Signal()
        self.writeData = VectSignal(self.DATA_WIDTH)
        # self.lock = Signal()
        self.waitRequest = Signal(masterDir=DIRECTION.IN)
        #self.writeResponseValid = Signal(masterDir=DIRECTION.IN)
        self.burstCount = VectSignal(log2ceil(self.MAX_BURST - 1))
        # self.beginBurstTransfer = Signal()
        

class AvalonMMRdDataAgent(RdSyncedAgent):
    """
    Simulation/verification agent for RdSynced interface inlined in in AvalomMM interface
    """
    def getRd(self, intf):
...
class AvalonAddrAgent(HandshakedAgent):
    ...
    def wrVld(self, wrFn, val):
        """
        read/write which are there used as valid signal are written from doWrite funtion
        """
    ...
    def doRead(self, s):
        ...
        if read.val:
            ...
            raise AssertionError("This funtion should not be called when data is not ready on interface")
...
class AvalonMMAgent(SyncAgentBase):
    def __init__(self, intf, allowNoReset=False):
        ...
        self.readAg = AvalonMMRdDataAgent(intf, allowNoReset=allowNoReset)
