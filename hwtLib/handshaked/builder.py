...
class HsBuilder(AbstractStreamBuilder):
    ...
    def to_axis(self, MAX_FRAME_WORDS=None, IN_TIMEOUT=None):
        ...
        def set_params(u: DataRdVldToAxi4Stream):
            u.DATA_WIDTH = DATA_WIDTH
            u.MAX_FRAME_WORDS = MAX_FRAME_WORDS
            u.IN_TIMEOUT = IN_TIMEOUT

        next_self = self._genericInstance(DataRdVldToAxi4Stream, "hsToAxi4S", set_params)

        from hwtLib.amba.axis_comp.builder import Axi4SBuilder
        return Axi4SBuilder(self.parent, next_self.end)
