...
class SSD1306CntrlProc(Unit, SSD1306CntrlProc_handlers):
    ...
    def _impl(self):
        ...
        delayDoneTick = self._sig("delayDoneTick")
        st = self.mainFsm(ir, delayDoneTick, self.spiCntrl.dataInDone)
        memData = self.memHandler(st, ip)

        self.pinsHandler(st, ir, vdd, vbat, res, dc)
        delayDoneTick ** self.dellayHandler(st, acc)
