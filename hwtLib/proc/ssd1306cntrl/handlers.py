...
class SSD1306CntrlProc_handlers():
    """
    Handlers of processor resources
    """
    ...
    def dellayHandler(self, st, acc):
        clkBuild = ClkBuilder(self, self.clk)
        msTick = clkBuild.timer(("ms_tick", self.FREQ // 1000))
        stT = st._dtype
        delayDoneTick = clkBuild.timerDynamic(acc[7:],
                                              enableSig=msTick & (st._eq(stT.DO_WAIT) | st._eq(stT.DO_WAIT_wait)))
        return delayDoneTick
